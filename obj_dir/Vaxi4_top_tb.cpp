...
void Vaxi4_top_tb::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vaxi4_top_tb::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
std::unique_ptr<VerilatedTraceConfig> Vaxi4_top_tb::traceConfig() const {
...
void Vaxi4_top_tb___024root__trace_decl_types(VerilatedVcd* tracep);

void Vaxi4_top_tb___024root__trace_init_top(Vaxi4_top_tb___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vaxi4_top_tb___024root__trace_decl_types(tracep);
    Vaxi4_top_tb___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}
...
VL_ATTR_COLD void Vaxi4_top_tb::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vaxi4_top_tb::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vaxi4_top_tb___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
