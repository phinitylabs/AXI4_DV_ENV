...
module axi4_slave_tb;
    ...
    // ASSERTION 1: BRESP must be valid when BVALID
    ...
    // ASSERTION 2: RRESP must be valid when RVALID
    ...
    property p_bid_match;
        @(posedge aclk) disable iff (!aresetn)
        bvalid |-> (bid == saved_awid);
    endproperty
    a_bid_match: assert property (p_bid_match)
        else $error("ASSERTION FAILED: BID mismatch");

    // ASSERTION 4: RID must match saved ARID
    logic [ID_WIDTH-1:0] saved_arid;
    always_ff @(posedge aclk or negedge aresetn) begin
        if (!aresetn)
            saved_arid <= '0;
    ...
    property p_rid_match;
        @(posedge aclk) disable iff (!aresetn)
        rvalid |-> (rid == saved_arid);
    endproperty
    a_rid_match: assert property (p_rid_match)
        else $error("ASSERTION FAILED: RID mismatch");

    // ASSERTION 5: BVALID requires preceding write address
    ...
    property p_bvalid_needs_write;
        @(posedge aclk) disable iff (!aresetn)
        bvalid |-> write_addr_received;
    endproperty
    a_bvalid_needs_write: assert property (p_bvalid_needs_write)
        else $error("ASSERTION FAILED: BVALID without write");

    // ASSERTION 6: RVALID requires preceding read address
    ...
    property p_rvalid_needs_read;
        @(posedge aclk) disable iff (!aresetn)
        rvalid |-> read_addr_received;
    endproperty
    a_rvalid_needs_read: assert property (p_rvalid_needs_read)
        else $error("ASSERTION FAILED: RVALID without read");

    // ASSERTION 7: DECERR for out-of-range write address
    ...
    property p_write_decerr;
        @(posedge aclk) disable iff (!aresetn)
        (bvalid && write_out_of_range) |-> (bresp == 2'b11);
    endproperty
    a_write_decerr: assert property (p_write_decerr)
        else $error("ASSERTION FAILED: Missing DECERR for out-of-range");

    // ASSERTION 8: BVALID requires WLAST received
    ...
    property p_bvalid_after_wlast;
        @(posedge aclk) disable iff (!aresetn)
        bvalid |-> wlast_received;
    endproperty
    a_bvalid_after_wlast: assert property (p_bvalid_after_wlast)
        else $error("ASSERTION FAILED: BVALID before WLAST");

    // ASSERTION 9: RLAST on final beat for single-beat reads
    property p_rlast_single;
        @(posedge aclk) disable iff (!aresetn)
        (arvalid && arready && arlen == 0) |=> (rvalid |-> rlast);
    endproperty
    a_rlast_single: assert property (p_rlast_single)
        else $error("ASSERTION FAILED: RLAST not set for single read");

    // ASSERTION 10: WLAST on final beat for single-beat writes
    property p_wlast_single;
        @(posedge aclk) disable iff (!aresetn)
        (awvalid && awready && awlen == 0) |=> (wvalid |-> wlast);
    endproperty
    a_wlast_single: assert property (p_wlast_single)
        else $error("ASSERTION FAILED: WLAST not set for single write");
    ...
    initial begin
        ...
        $display("================================================================");
        $display(" AXI4 Slave Testbench - Golden");
        $display("================================================================");
        reset_dut();
