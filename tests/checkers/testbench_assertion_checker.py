...
class AssertionChecker:
    ...
    def find_assertions(self) -> Dict[str, List[str]]:
        ...
        # Pattern for manual assertions using $display/$error for assertion reporting
        # Matches: $display("ASSERTION PASSED: ..."), $error("ASSERTION FAILED: ...")
        # Also matches: $display("ASSERTION 1 PASSED"), etc.
        manual_assertion_patterns = [
            r'\$display\s*\([^)]*ASSERTION[^)]*PASSED[^)]*\)',
            r'\$error\s*\([^)]*ASSERTION[^)]*FAILED[^)]*\)',
            r'\$display\s*\([^)]*ASSERTION[^)]*FAILED[^)]*\)',
        ]
        manual_assertions = []
        for pattern in manual_assertion_patterns:
            matches = re.findall(pattern, content, re.IGNORECASE)
            manual_assertions.extend(matches)
