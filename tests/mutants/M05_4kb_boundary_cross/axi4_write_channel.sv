// =============================================================================
// MUTANT M05: 4KB Boundary Crossing Bug
// Bug: No check for 4KB boundary - address can wrap incorrectly across 4KB
// =============================================================================
...
#(
  ...
  logic [ADDR_WIDTH-1:0]  start_addr_page;  // Start address page (bits [31:12])
  logic [7:0]             burst_len;
  ...
  always_ff @(posedge clk or negedge aresetn) begin
    if (!aresetn) begin
      ...
      start_addr_page <= '0; has_boundary_error <= 1'b0;
    end else begin
      case (state)
        IDLE: begin
          if (awvalid && awready) begin
            ...
            start_addr_page <= awaddr[31:12];
            beat_count <= '0; has_decode_error <= decode_error;
            has_wlast_error <= 1'b0; has_boundary_error <= 1'b0;
          end
        end
        DATA_BURST: begin
          if (wvalid && wready) begin
            ...
            // BUG: No 4KB boundary check - should flag error if crossing 4KB
            // Missing: if (current_addr[31:12] != start_addr_page) has_boundary_error <= 1'b1;
            
            case (burst_type)
              BURST_FIXED: ;
              BURST_INCR: current_addr <= current_addr + addr_incr;  // Can cross 4KB!
              BURST_WRAP: begin
  ...
  always_comb begin
    ...
    // BUG: No boundary error check in response - should return SLVERR
    // Missing: else if (has_boundary_error) resp_value = RESP_SLVERR;
