// =============================================================================
// MUTANT E07: RID Stale Bug
// Bug: RID is not updated for back-to-back transactions - uses stale ID
// =============================================================================
...
#(
  ...
  logic [ID_WIDTH-1:0]    rid_reg;  // BUG: Separate register that doesn't update properly
  ...
  // BUG: rid_reg only updates on first transaction, not subsequent ones
  always_ff @(posedge clk or negedge aresetn) begin
    if (!aresetn) begin
      rid_reg <= '0;
    end else if (arvalid && arready && (rid_reg == '0)) begin
      // BUG: Only updates if rid_reg is zero - stale after first transaction!
      rid_reg <= arid;
    end
  end
  ...
  assign rid    = rid_reg;  // BUG: Uses stale rid_reg instead of txn_id
