// =============================================================================
// MUTANT E05: Reset BVALID Bug
// Bug: BVALID is not properly deasserted after reset (state machine bug)
// =============================================================================
...
#(
  ...
  // BUG: bvalid_reg doesn't reset properly
  logic                   bvalid_reg;
  ...
  // BUG: bvalid_reg is not reset - will be X or random after reset!
  always_ff @(posedge clk) begin  // BUG: Missing reset in sensitivity list
    if (state == SEND_RESP || state == WAIT_BREADY)
      bvalid_reg <= 1'b1;
    else
      bvalid_reg <= 1'b0;
  end
  ...
  assign bvalid = bvalid_reg;  // BUG: Uses registered version that doesn't reset
