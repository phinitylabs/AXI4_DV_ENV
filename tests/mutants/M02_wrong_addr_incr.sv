// Mutant M02: Wrong address increment - always increments by 1 instead of burst size
...
#(
  ...
  // BUG: Wrong increment - always 1 instead of based on burst size
  logic [ADDR_WIDTH-1:0] addr_incr;
  assign addr_incr = 1;  // Should be (1 << burst_size)
  ...
  always_comb begin
    next_state = state;
    case (state)
      IDLE: if (arvalid && arready) next_state = ADDR_RECV;
      ADDR_RECV: next_state = MEM_READ;
      MEM_READ: next_state = MEM_WAIT;
      MEM_WAIT: if (mem_rd_valid || has_decode_error) next_state = SEND_DATA;
      SEND_DATA: begin
        if (rready) begin
          if (beat_count == burst_len) next_state = IDLE;
          else next_state = MEM_READ;
        end else next_state = WAIT_RREADY;
      end
      WAIT_RREADY: begin
        if (rready) begin
          if (beat_count == burst_len) next_state = IDLE;
  ...
  always_ff @(posedge clk or negedge aresetn) begin
    if (!aresetn) begin
      txn_id <= '0; current_addr <= '0; burst_len <= '0; beat_count <= '0;
      burst_size <= '0; burst_type <= '0; wrap_boundary <= '0;
      has_decode_error <= 1'b0; data_reg <= '0;
    end else begin
      case (state)
        IDLE: begin
          if (arvalid && arready) begin
            txn_id <= arid; current_addr <= araddr; burst_len <= arlen;
            burst_size <= arsize; burst_type <= arburst;
            wrap_boundary <= (araddr & ~(wrap_size - 1));
            beat_count <= '0; has_decode_error <= decode_error;
          end
        end
        MEM_WAIT: if (mem_rd_valid) data_reg <= mem_rd_data;
  ...
  assign rresp  = has_decode_error ? RESP_DECERR : RESP_OKAY;
