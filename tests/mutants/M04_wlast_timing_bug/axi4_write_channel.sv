// =============================================================================
// MUTANT M04: WLAST Timing Bug
// Bug: WLAST check is off-by-one (expects WLAST one beat early)
// =============================================================================
...
#(
  ...
  always_comb begin
    ...
    case (state)
      ...
      // BUG: State machine checks beat_count instead of wlast for transition
      DATA_BURST: if (wvalid && wready && (beat_count >= burst_len)) next_state = SEND_RESP;  // BUG!
      ...
      case (state)
        ...
        DATA_BURST: begin
          if (wvalid && wready) begin
            ...
            // BUG: Off-by-one check - should be (beat_count == burst_len)
            if ((beat_count == burst_len - 1) && !wlast) has_wlast_error <= 1'b1;  // BUG!
            case (burst_type)
              BURST_FIXED: ;
